...
class SurveyController {
    ...
    private function _searchText($searchOption,$page,$count){
        ...
        if($searchOption['sort'] == 'ms'){
            ...
            if($searchOption['text'] != ''){
                ...
                $baseSurveys = SvSurvey::where('author_user_id', $searchOption['target_user_id'])->where('description','like', "%$text%")->get();
            }
            ...
            if($searchOption['text'] != ''){
                $text = $searchOption['text'];
                $answers = SvUserIdSurveyRelation::where('user_id', $searchOption['target_user_id'])->where('description','like', "%$text%")->get();
            }
            ...
            if($searchOption['text'] != ''){
                $text = $searchOption['text'];
                $baseSurveys =  SvSurvey::where('description','like', "%$text%")->get();
            }
        }
    ...
    public function webGetSurvey(Request $request) {
       ...
       if($request->error == 1){
        ...
        return view(($data['voted'] or $data['is_end'] or $data['is_my_survey'])? 'votedsurvey':'survey', ['message' => $message,'data' =>$data, 'userdata' => $userdata]);
    }
    ...
    private function _getSurvey(Request $request){
        ...
        $is_my_survey = ($survey->author_user_id == $user_id);   //自分が作成したか
